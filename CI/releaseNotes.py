...
def allPulls(releaseDate):
    ...
    content = ghApiClient.readUrl('repos/frantuma/nagger-core/pulls?state=closed&base=master&per_page=100')
    for l in content:
...
def lastReleaseDate(tag):
    content = ghApiClient.readUrl('repos/frantuma/nagger-core/releases/tags/' + tag)
...
def addRelease(release_title, tag, content):
    ...
    content = ghApiClient.postUrl('repos/frantuma/nagger-core/releases', payload)
    return content
